...
class StaffController extends Controller
{
	...
	public function getReceipt(Request $request, $receipt_id)
	{
		...
		$receiptrecord = Receipt::find($receipt_id);

		if($receiptrecord->module == 'shengzhupai')
		{
			$receipt = Receipt::join('shengzhupai', 'shengzhupai.id', '=', 'receipt.module_id')
						->join('staff','receipt.staff_id','=','staff.staff_id')
						->join('devotee', 'devotee.devotee_id', '=', 'shengzhupai.focusdevotee_id')
						->select('receipt.*', 'devotee.chinese_name', 'devotee.*', 'shengzhupai.*','staff.first_name','staff.last_name')
						->where('receipt.receipt_id', $receipt_id)
						->get();

						return view('staff.receipt_shengzhupai', [
 ...
 }elseif($wanyuanshenghuis[0]['type'] == '3')
 {
	...
	public function getPreOrderShengzhupai()
	{
		$today = Carbon::today();
	...
	public function getPreOrderLingwei()
	{
		$today = Carbon::today();
...
public function getRmkbei()
	...
	public function getWanyuanshenghuiprint4()
	{
		...
		public function postShengzhupai(Request $request)
		{
	        ...
	        "total_gst" => $input['total_aftergst']- $input['total'],
	        "total_aftergst" => $input['total_aftergst'],
	        "total_afterdiscount" => $input['total_afterdiscount'],
			  "focusdevotee_id" => $input['focusdevotee_id'],
			  "slot_id" => $input['slot_id'],
				 "devotee_selected" => $input['devotee_selected'],
				  "otheroption_text" => $input['otheroption_text']
			...
			$shengzhupai = Shengzhupai::create($data);
			  ...
			  $data_receipts = [
							    ...
							    "trans_date" => Carbon::now(),
							    "description" => "神主牌:".$input['ss_blk']."-".$input['ss_level']."-".$input['ss_number'],
							    "amount" => $input['total'],
							    "module" => "shengzhupai",
		...
		// Get Receipt History
			$shengzhupai_receipts = Receipt::leftjoin('shengzhupai', 'shengzhupai.id', '=', 'receipt.module_id')
									...
									->select('receipt.*','shengzhupaislots.*', 'devotee.chinese_name','devotee.contact','devotee.nric', 'devotee.devotee_id')
									->get();
	      ...
	      Session::put('shengzhupai_receipts', $shengzhupai_receipts);
				...
				//set slot number to 0 and to that person id
				$shengzhupaislotrecord = Shengzhupaislots::find($input['slot_id']);
			$shengzhupaislotrecord->status = 'purchased';
			$shengzhupaislotrecord->devotee_id = $input['focusdevotee_id'];
			$shengzhupaislotrecord->save();
			...
			$data = [
				...
				"devotee_id" => $input['focusdevotee_id'],
				"type" => "journal",
				"total_debit_amount" => $input['total'],
				"total_credit_amount" => $input['total']
			];
			...
			$data = [
				"glcode_id" => 247,
				"debit_amount" => null,
				"credit_amount" => $input['total'],
				"journalentry_id" => $journalentry->journalentry_id
			];

			JournalEntryItem::create($data);

return \Redirect::route('receipt-page', $shengzhupaireceipt->receipt_id)->with('message', '神主牌 is successfully created');
	    }

			// Post Sheng Zhu Pai
			public function postPreOrderShengzhupai(Request $request)
			{
				$input = array_except($request->all(), '_token');
		...
		public function postLingwei(Request $request)
		{
			...
			// Post Ling Wei
		public function postPreOrderLingwei(Request $request)
		{
			$input = array_except($request->all(), '_token');
