...
class ReportController extends Controller
{
  ...
  public function getSummarySettlementReport()
  {
    ...
    $glcode = GlCode::whereIn('glcode_id', array(247, 251, 252, 119, 134))
  ...
  public function getReportDetail(Request $request)
  {
    ...
    // INCOME
    $income_glcode = Glcode::where('glcodegroup_id', 6)
  ...
  public function getCashflowReportDetail(Request $request)
  {
    ...
    if(isset($input['year']) && isset($nmonth))
    {
      ...
      $ocbc_account = GlCode::leftjoin('glcodegroup', 'glcode.glcodegroup_id', '=', 'glcodegroup.glcodegroup_id')
                        ...
                        ->select(DB::raw('SUM(IF(YEAR(payment_voucher.date) =' . $input['year'] . ', payment_voucher.total_debit_amount, 0))' . $input['month']),
                        'glcode.accountcode', 'glcode.type_name')
                        ->get();
      ...
      $total_ocbc_account = GlCode::leftjoin('glcodegroup', 'glcode.glcodegroup_id', '=', 'glcodegroup.glcodegroup_id')
                              ...
                              ->where('payment_voucher.cheque_account', 7)
                              ->where(DB::raw('YEAR(payment_voucher.date)'), '=', $input['year'])
                              ->select(DB::raw('SUM(IF(YEAR(payment_voucher.date) =' . $input['year'] . ', payment_voucher.total_debit_amount, 0)) AS total'),
                                'glcode.accountcode', 'glcode.type_name')
                              ->get();
      ...
      //                 ->select(DB::raw('SUM(IF(YEAR(pettycash_voucher.date) =' . $input['year'] . ', pettycash_voucher.total_debit_amount, 0))' . $input['month']),
      //                   'glcode.accountcode', 'glcode.type_name')
      //                 ->get();
      //
      ...
      //                        ->leftjoin('pettycash_voucher', 'glcode.glcode_id', '=', 'pettycash_voucher.glcode_id')
      //                        ->where(DB::raw('YEAR(pettycash_voucher.date)'), '=', $input['year'])
      //                        ->select(DB::raw('SUM(IF(YEAR(pettycash_voucher.date) =' . $input['year'] . ', pettycash_voucher.total_debit_amount, 0)) AS total'),
      //                         'glcode.accountcode', 'glcode.type_name')
      //                        ->get();
    ...
    else
    {
      ...
      {
        $expenses = Glcode::leftjoin('glcodegroup', 'glcode.glcodegroup_id', '=', 'glcodegroup.glcodegroup_id')
                    ->leftjoin('payment_voucher_item', 'glcode.glcode_id', '=', 'payment_voucher_item.glcode_id')

                    ->where('payment_voucher_item.glcode_id', $expenses_glcode[$i]->glcode_id)
                    ->where(DB::raw('YEAR(payment_voucher_item.created_at)'), '=', $input['year'])
                    ->select(DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 1, payment_voucher_item.debit_amount, 0)) AS Jan'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 2, payment_voucher_item.debit_amount, 0)) AS Feb'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 3, payment_voucher_item.debit_amount, 0)) AS Mar'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 4, payment_voucher_item.debit_amount, 0)) AS Apr'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 5, payment_voucher_item.debit_amount, 0)) AS May'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 6, payment_voucher_item.debit_amount, 0)) AS Jun'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 7, payment_voucher_item.debit_amount, 0)) AS July'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 9, payment_voucher_item.debit_amount, 0)) AS Sep'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 10, payment_voucher_item.debit_amount, 0)) AS Oct'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 11, payment_voucher_item.debit_amount, 0)) AS Nov'),
                    DB::raw('SUM(IF(MONTH(payment_voucher_item.created_at) = 12, payment_voucher_item.debit_amount, 0)) AS December'),
                    'glcode.type_name')
                    ->get();
      ...
      $ocbc_account = GlCode::leftjoin('glcodegroup', 'glcode.glcodegroup_id', '=', 'glcodegroup.glcodegroup_id')
                      ...
                      ->where('payment_voucher.cheque_account', 7)
                      ->where(DB::raw('YEAR(payment_voucher.date)'), '=', $input['year'])
                      ->select(DB::raw('SUM(IF(MONTH(payment_voucher.date) = 1, payment_voucher.total_debit_amount, 0)) AS Jan'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 2, payment_voucher.total_debit_amount, 0)) AS Feb'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 3, payment_voucher.total_debit_amount, 0)) AS Mar'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 4, payment_voucher.total_debit_amount, 0)) AS Apr'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 5, payment_voucher.total_debit_amount, 0)) AS May'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 6, payment_voucher.total_debit_amount, 0)) AS Jun'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 7, payment_voucher.total_debit_amount, 0)) AS July'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 8, payment_voucher.total_debit_amount, 0)) AS Aug'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 9, payment_voucher.total_debit_amount, 0)) AS Sep'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 10, payment_voucher.total_debit_amount, 0)) AS Oct'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 11, payment_voucher.total_debit_amount, 0)) AS Nov'),
                      DB::raw('SUM(IF(MONTH(payment_voucher.date) = 12, payment_voucher.total_debit_amount, 0)) AS December'),
                      'glcode.type_name')
                      ->get();
      ...
      //                   ->where(DB::raw('YEAR(pettycash_voucher.date)'), '=', $input['year'])
      //                   ->select(DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 1, pettycash_voucher.total_debit_amount, 0)) AS Jan'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 2, pettycash_voucher.total_debit_amount, 0)) AS Feb'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 3, pettycash_voucher.total_debit_amount, 0)) AS Mar'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 4, pettycash_voucher.total_debit_amount, 0)) AS Apr'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 5, pettycash_voucher.total_debit_amount, 0)) AS May'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 6, pettycash_voucher.total_debit_amount, 0)) AS Jun'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 7, pettycash_voucher.total_debit_amount, 0)) AS July'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 8, pettycash_voucher.total_debit_amount, 0)) AS Aug'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 9, pettycash_voucher.total_debit_amount, 0)) AS Sep'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 10, pettycash_voucher.total_debit_amount, 0)) AS Oct'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 11, pettycash_voucher.total_debit_amount, 0)) AS Nov'),
      //                   DB::raw('SUM(IF(MONTH(pettycash_voucher.date) = 12, pettycash_voucher.total_debit_amount, 0)) AS December'),
      //                   'glcode.type_name')
      //                   ->get();
  ...
  public function getTrialBalanceReportDetail(Request $request)
  {
    ...
    $income_glcode = Glcode::where('glcodegroup_id', 6)->get();
  ...
  public function postSummarySettlementReport(Request $request)
  {
    ...
    $glcode_array = [247, 251, 252, 119, 134];
    $total_cash = 0;
    $total_nets = 0;
    $total_cheque = 0;
    $total_amount = 0;
    ...
    if($input['staff_id'] == 0)
    {
      if($input['type'] == 0)
      {
        for($i = 0; $i < $total_staff; $i++)
        {
          ...
          {
            $result = Receipt::rightjoin('glcode', 'receipt.glcode_id', '=', 'glcode.glcode_id')
                      ->where('receipt.staff_id', $staff_array[$i])
                      ->where('receipt.trans_date', $date)
                      ->where('receipt.glcode_id', $glcode_array[$j])
                      ->select(DB::raw('SUM(IF(receipt.mode_payment = "cash", receipt.amount, 0)) AS cash'),
                      DB::raw('SUM(IF(receipt.mode_payment = "cheque", receipt.amount, 0)) AS cheque'),
                      DB::raw('SUM(IF(receipt.mode_payment = "nets", receipt.amount, 0)) AS nets'),
                      DB::raw('SUM(IF(receipt.mode_payment = "receipt", receipt.amount, 0)) AS receipt'),
                      'glcode.type_name')
                      ->get();
  ...
  public function postSettlementReport(Request $request)
  {
    ...
    if($input['staff_id'] == 0)
    {
      if($input['type'] == 0)
      {
        $result = Receipt::leftjoin('glcode', 'receipt.glcode_id', '=', 'glcode.glcode_id')
                  ->leftjoin('devotee', 'receipt.devotee_id', '=', 'devotee.devotee_id')
                  ->leftjoin('user', 'receipt.staff_id', 'user.id')
                  ->where('receipt.trans_date', $date)
                  ->select('receipt.*', 'devotee.chinese_name', 'receipt.mode_payment',
