...
class OperatorController extends Controller
{
	  ...
	  $devotee = Devotee::leftjoin('member', 'devotee.member_id', '=', 'member.member_id')
		...
		// Get Receipt History
			$shengzhupai_receipts = Receipt::leftjoin('shengzhupai', 'shengzhupai.id', '=', 'receipt.module_id')
									...
									->where('shengzhupai.focusdevotee_id', $devotee_id)
    ...
    if(count($xiaozai_receipts) > 0)
	  ...
	  Session::put('focus_devotee', $devotee);
		...
		return redirect()->route('get-shengzhupai-page', [
			...
			else
		  {
				...
				if($devotee_id != null)
		    {
		      ...
		      if(isset($input['special_remark'][0]))
		      {
				...
				if(Session::has('shengzhupai_receipts')) { Session::forget('shengzhupai_receipts'); }
...
public function jsonGetOtherOption(Request $request)
	{
			...
			$selectionoptiondata = $selectionoption;
			$autocomplete =[];
			if($selectionoptiondata){
					foreach ($selectionoptiondata as $key => $value) {
							$autocomplete['suggestions'][] = ['value'=>$value['name'].'-$'.$value['price'], 'data' => ['price'=> $value['price'], 'id'=> $value['id'], 'name'=>$value['name']]];
					}
			} else {
					$autocomplete['suggestions'][] = ['value'=>'Sorry, no matching results', 'data' => ['price'=>'0', 'id'=> '', 'name'=>'']];
			}

			return Response::json($autocomplete);
	}

	public function getDevoteeInfo(Request $request)
	{
		$nric = $_GET['nric'];

		$devotee_info = Devotee::where('nric', $nric)
												 ->get();

		return response()->json(array(
			'devotee_info' => $devotee_info
		));
	}

	public function getDevoteeNRIC(Request $request)
	{
			$member = Input::get('term');

			$results = array();

			$queries = Devotee::where('nric', 'like', '%'.$member.'%')
								 ->take(5)
								 ->get();
